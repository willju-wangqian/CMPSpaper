...
\abstract{%
In this paper, we introduce the R package cmpsR, an open-source
...
\item
  ...
  \item
    ...
    \tightlist
    \item
      If \texttt{npeaks.set} is an integer vector of length 1, for
      example \texttt{npeaks.set\ =\ 5}, the positions of the top five
      peaks in the cross-correlation curve are identified as candidate
      positions for registration.
    \item
      If \texttt{npeaks.set} is an integer vector of length more than 1,
      for example \texttt{npeaks.set\ =\ c(5,\ 3,\ 1)}, the
      ...
      cross-correlation function. \autoref{fig:seg_all} shows an example
      of three levels of basis segment 6 and their corresponding
    ...
    \item
      ...
      \texttt{seg\_outlength}, and \texttt{nseg} determines the segment
      ...
      resulting list is called \texttt{ccr.list}.
    \end{itemize}
  \item
    ...
    \texttt{Tx} in all segment levels. Note that in \citet{cmps}, a
...
In the first example, \texttt{npeaks.set} is a vector of three integers,
