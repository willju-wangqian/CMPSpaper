...
#################################################
rf_hamby44_results$rf.table[[1]] %>% names()

# generate plots
metric_plot_helper(rf_hamby44_results$rf.table[[1]], "diff", scaled = TRUE,
                   subtitle = "diff plot for rf score",
                   breaks = seq(0,1,0.1), binwidth = 0.05)

metric_plot_helper(rf_hamby44_results$rf.table[[1]], "maxbar", scaled = TRUE,
                   subtitle = "max plot for rf score",
                   breaks = seq(0, 1, 0.1), binwidth = 0.05) 

metric_plot_helper(rf_hamby44_results$rf.table[[1]], "diff.med", scaled = TRUE,
                   subtitle = "diff with median plot for rf score",
                   breaks = seq(0, 1, 0.1), binwidth = 0.05)

with(rf_hamby44_results$rf.table[[1]], {
