...
In this paper we present an open-source implementation of the algorithm of the Congruent Matching Profile Segments (CMPS) method. @cmps developed the CMPS method for "objective comparison of striated tool marks" and demonstrated its usage in examples of comparing bullet signature correlations. Although @cmps introduced the algorithm of the CMPS method in their paper, \hh{they did not release an implementation of their method}. Thus, our effort here is to introduce the CMPS method to the `R` community and provide an open-source implementation of the algorithm for people to use and make further improvements. Our implementation is wrapped up into  `R` package  `CMPS`, which is available on GitHub at \url{https://github.com/willju-wangqian/CMPS}. 
