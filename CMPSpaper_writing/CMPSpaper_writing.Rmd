...
rf.tt <- rf.tt %>% pivot_longer(
  cols = c("cmps.diff", "cmps.max", "cmps.maxbar"),
  names_to = "Metric"
)
...
labels <- expression(bar(CMPS)[diff], {bar(CMPS)^symbol("*")}[diff], CMPS[max], bar(CMPS)[max])
...
```{r result1_44, echo=FALSE, out.width="400px", fig.cap="Distribution of $\\mathrm{CMPS_{max}}$ and $\\mathrm{\\overline{CMPS}_{max}}$ for Hamby 44; outliers are removed in bullet signatures; \\texttt{seg\\_length = 61}, \\texttt{Tx = 30}, \\texttt{npeaks.set = c(5,3,1)} ", fig.height=3}
# [4] "span1=0.25, span2=0.03, rm_outliers = TRUE, 5025, 531"  
# knitr::include_graphics("img/hamby252_v1.png", dpi = 100)
# CMPS_hamby252_results$plot[[2]]
...
```{r param_seg_plot, echo=FALSE, out.width="400px", fig.cap="Comparison of results from the CMPS algorithm based on different basis segment lengths (parameter \\texttt{seg\\_length}). Only the $\\mathrm{CMPS_{max}}$ metric suggests that the default values for the basis segment length result in the best separation. Better separation is achieved based on the modified CMPS metrics, inclusing the newly suggested ones. For Hamby 252 these metrics agree that a segment length of 75 and a segment length of 122 for Hamby 44 yield better results", fig.height=3, warning = FALSE}
p.seg_plot 
```


```{r param_npeak_plot, echo=FALSE, out.width="400px", fig.cap="Comparison of CMPS results based on different strategies of number of peak selections. \\hh{5-3-1 should be next to 5-3-1*}. Starred results compare CMPS performance with results published in the literature. ", fig.height=3, warning = FALSE}
p.npeak_plot
```
...
\wj{One interesting observation is that cmps.max works pretty well for 531 in nist's paper... do we need to include this?XXX} \hh{no need}
...
<!--A random forest model can be used for both classification and regression. 
It is an ensemble learning method that takes advantage of multiple predictive features.-->
...
In this paper we present the CMPS package, an open-source implementation of the Congruent Matching Profile Segments (CMPS) algorithm [@cmps], and apply it to two datasets in Hamby study [@hamby] to show its potential for further research.
The CMPS algorithm was proposed by NIST in 2019 and was made for objective tool marks comparisons.
We introduce the basic logic of the CMPS algorithm and lay out its implementation in the CMPS package.
We also showcase the functionality of the CMPS package with a small dataset example that is included in the package.
In the CMPS package we  implement some graphing tools for users to visualize results and to gain a better understanding of both the algorithm and the results.

Additionally, we apply the CMPS package to two datasets in Hamby study (Hamby set 252 and Hamby set 44). The results obtained are promising:  we were both able to reproduce the results in @cmps qualitatively and achieve a clear separation between the known-match (KM) comparisons and known-nonmatch (KNM) comparisons in another bullet study.
However, the results of Hamby set 44 suggest that the CMPS method is sensitive to the choice of parameters in the algorithm, such as the length of basis segments and the number of peaks detected at each segment level.
...
The CMPS package provides an open-source implementation and introduces the CMPS algorithm to the `R` community. 

\hh{XXX Which 'It' are you referring to?}
