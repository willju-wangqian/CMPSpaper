...
The command `get_segs(x, len=50)` implements this step:  it takes a bullet signature  in the format of a numeric vector and cuts it into consecutive, non-overlapping and equal-length segments of length `len`.

2. **register each basis segment on the comparison profile:** 
Calculate the cross-correlation curve between a basis segment and the comparison profile (using function `get_ccf4`).
A position is considered  "good" if the cross-correlation between the basis segment and the comparison is above a pre-defined threshold. \hh{XXX what is the name of the parameter?} 
The best registration position is decided upon using  a multi-peak inspection with or without different segment lengths:

    2.1. **multi-peak inspection (without different segment lengths):** each basis segment identifies positions of the top X \hh{parameter name?} highest peaks in the cross-correlation curve. Small segment lengths might indicate false positive peaks. Therefore all X peaks are considered as candidates for the "best" registration position.

    2.2. **multi-peak inspection with different segment lengths:** for this version of the CMPS algorithm, each basis segment finds only one "best" registration position. This "best" registration position can be found by conducting multi-peak inspection multiple rounds. The first round is executed as stated in step 2.1. After that first round, the segment length is increased, a new cross-correlation curve is computed, and another set of  peaks in the cross-correlation curve is identified together with the candidate positions. Typically, fewer number of peaks are identified in this round (because the increased segment length reduces the chance of false positive peaks). This process can be repeated multiple times. In the last round, only one highest peak should be identified. If the same or 'similar enough' (see step 3) position can be identified for all different segment length, this position will be selected as the "best" registration position and is called the "consistent correlation peak" (ccp).
    
3. **determine the congruent registration position**:
candidate positions across different segements are compared using a small tolerance \hh{XXX parameter for this} and basis segments with a congruent registration position are determined. These segments are called the "congruent matching profile segments" (CMPS), and the number of CMPS is the CMPS score of the comparison.
