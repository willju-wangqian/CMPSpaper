...
We now introduce a measure called "variance ratio" that evaluates the separation between two groups. Let $n_k$ denote the number of observations in group $k$ and $y_{kl}$ denote the $l$-th observation in group $k$, for $k = 1,2$ and $l = 1, \dots, n_k$. Let $\bar{y}_{k.}$ denote the mean value in group $k$ and $\bar{y}_{..}$ denote the overall mean. Then we can define the variance ratio $V$ as:


$$
V = \frac{\sum n_k (\bar{y}_{k.} - \bar{y}_{..})^2}{\sum_k \sum_l^{n_k} (y_{kl} - \bar{y}_{k.})^2 }
$$

This is called "variance ratio" since the numerator evaluates the "variance" between the two groups, and the denominator evaluates the "variance" within each group. \wj{Note that this quantity is not equal to the F-statistic in one-way ANOVA since we ignore the degrees of freedom of both the numerator and the denominator. We do not need the degrees of freedom because all the variance ratio values compared in the next sections have exactly the same number of observations in both goups (the known-match group and the known-nonmatch group), resulting in exactly the same degrees of freedom.}
