...
3. **determine the congruent registration position:** Each candidate position corresponding to each of the basis segments gets one `vote`. Votes for candidate positions within a tolerance of `Tx` are tallied and the position with the highest number of votes gets chosen as the *congruent registration position*. Basis segments with a congruent registration position are called "congruent matching profile segments" (CMPS). The total number of CMPS is the CMPS score of the comparison. \hh{XXX what happens when there are ties after the vote?}
