...
Besides the previously discussed parameters `seg_length`, `npeaks.set`, and `Tx` \hh{the function `extract\_feature\_cmps` allows for the following input from users}:
...
$$
\mathrm{CMPS_{max}} =  \max_{ij} c_{ij} = 17
$$
...
Note that for Hamby set 44, one unit represents 1.5625 \textmu m so setting `seg_length = 50` makes each basis segment have length 78.125 \textmu m. And `Tx = 25` makes the tolerance zone be $\pm 39.0625$ \textmu m. This is to mimic the parameters used in @cmps.
...
\autoref{fig:result1_44} shows the results for Hamby set 44. Again, we are able to see a clear separation between the KM comparisons and the KNM comparisons, but it would be better if the separation can be further enlarged. \wj{Table x} shows some summary statistics for Hamby set 44. 
