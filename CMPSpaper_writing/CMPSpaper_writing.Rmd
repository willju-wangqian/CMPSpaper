...
```{r param_npeak_plot, echo=FALSE, out.width="400px", fig.cap="Comparison of results from the CMPS algorithm based on different number of peaks identified at each segment level (parameter \\texttt{npeaks.set}). ", fig.height=3, warning = FALSE}
p.npeak_plot
```
...
\hh{edits up to here}
Additionally, we fixed `seg_length` to be the default values and examined different choices of `npeaks.set`.
As shown in \autoref{fig:param_npeak_plot}, `npeaks.set = c(5,3,1)`, the default value of `npeaks.set` leads to promising results in terms of the sum of squares ratio; however, other choices of `npeaks.set` could result in higher value of the sum of squares ratio.

As for comparing different CMPS metrics, the results might suggest that using $\mathrm{CMPS_{max}}$ produces the least amount of separation, compared with other CMPS metrics.
They might also suggest that $\mathrm{\overline{CMPS}_{diff}}$ and $\mathrm{\overline{CMPS^*}_{diff}}$, the two metrics we proposed above, can lead to equally good or even better results as $\mathrm{\overline{CMPS}_{max}}$ can lead to. 
Since $\mathrm{\overline{CMPS^*}_{diff}}$ relies on the scaled CMPS scores that is more comparable to other similarity scores and summarizes the out-of-phase background CMPS scores, we might choose $\mathrm{\overline{CMPS^*}_{diff}}$ over other CMPS metrics. 
...
The same set of parameters might work for one data set, but not work for another. 
...
@cmps presents the histograms of $\mathrm{CMPS_{max}}$ and $\mathrm{\overline{CMPS}_{max}}$ for `npeaks.set = 5` and `npeaks.set = c(5,3,1)` with Hamby 252.
We computed the sum of squares ratios according to those histograms and included the results in \autoref{fig:param_npeak_plot} as well.
...
\wj{One interesting observation is that cmps.max works pretty well for 531 in nist's paper... do we need to include this?XXX}
...
In \autoref{fig:param_npeak_plot}, we also included the sum of squares ratios computed from the random forest scores for different metrics. 
...
The random forest model we used here was presented in XXX and trained at the Center for Statistics and Applications in Forensic Evidence (CSAFE).
Similarly to the CMPS algorithm, this random forest model produces a random forest score that lies between the interval $(0, 1)$ for each land-by-land comparison.
So the random forest scores are more comparable to the scaled CMPS scores.
...
This might suggest researchers to include the CMPS score as an additional feature in the random forest model training. 
