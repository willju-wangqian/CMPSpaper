...
result.container$npeak <- c("5*", "5-3-1*", "5*", "5-3-1*")
...
```{r param_seg_plot, echo=FALSE, out.width="400px", fig.cap="Some caption", fig.height=3}
p.seg_plot 
```

```{r param_npeak_plot, echo=FALSE, out.width="400px", fig.cap="Some caption", fig.height=3}
p.npeak_plot
```


\wj{new added the following}

For both Hamby set 252 and 44, we investigated the effects of different sets of parameters.
More specifically, we fixed the parameter `npeaks.set` that controls the number of peaks at each segment level to be `npeaks.set = c(5,3,1)` and modified the parameter `seg_length` that controls the length of a basis segment. 
As shown in \autoref{fig:param_seg_plot}, the default values of `seg_length` that we used for Hamby 252 and Hamby 44 (50 for Hamby 252 and 61 for Hamby 44, which represent 78.125 \textmu m and 78.69 \textmu m, respectively) result in high values of the sum of squares ratio, no matter which CMPS metrics we are using, when `npeaks.set = c(5,3,1)`. 
However, better choices also exist.
For example, \autoref{fig:param_seg_plot} suggests that basis segment length of 75 might be a better choice than the default value for Hamby 252; basis segment length of 122 result in a higher value of sum of squares ratio for Hamby 44.
