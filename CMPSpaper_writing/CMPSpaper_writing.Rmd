...
seg.plot <- cmps_segment_plot(
  cmps_with_multi_scale, seg.idx = 6
  )
...
\begin{figure}
\begin{subfigure}[t]{\textwidth}
\caption{\label{fig:segplot3}put segment\_plot and scale\_ccf\_plot of all scales together. We are identifying five highest peaks at scale 1, three peaks at scale 2, and one peak at scale 3 since npeaks.set = c(5, 3, 1). Putting these plots together can help better understand why segment 6 is not a congruent matching profile segment. Here we can also see clearly that increasing segment length can reduce the number of false positive peaks.}
...
```
\end{subfigure}
\caption{\label{fig:seg_all} some text.}
\end{figure}
