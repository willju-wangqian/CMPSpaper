...
\begin{figure}[htb]
```{r, opts.label="codefig", echo=TRUE, out.width='\\textwidth', warning = FALSE, fig.cap=" "}
...
```
\caption{\label{segplot} segment\_plot for segment 6 at scale 1. The original position of segment 6 is indicated by the solid black line. Positions where the segment achieves the 5 highest cross-correlations are indicated by the dashed line segments.}
\end{figure}
