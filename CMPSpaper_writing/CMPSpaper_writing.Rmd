...
\wj{The white horizontal line in \autoref{fig:process}(a) indicates this fixed height selected by an algorithm, and \autoref{fig:process}(b) shows the cross-sectional view of the bullet scan at this height. Then we can acquire the crosscut data (`ccdata`) of the scan at this height. \autoref{fig:process}(c) shows the typical pattern of the crosscut data, where "wedges" can be found on the left and right side and indicate the boundary between the land engraving and the groove engravings. The next step is to locate and chop these wedge areas so that only the data of the land engraving in the middle is kept (as shown in \autoref{fig:process}(d)). With the groove engravings excluded, we can then smooth out the land engraving curvature (\autoref{fig:process}(e)) using LOESS (locally estimated scatter plot smoothing) [@loess] and obtain the bullet signature (\autoref{fig:process}(f)). \autoref{fig:process} illustrates this framework. Sometimes the algorithm in our framework cannot determine the valid location of the crosscut or the grooves due to the deformation of a bullet, so we also developed a `R` Shiny App called `bulletinspectR` to identify and correct those errors. Note that NIST researchers used a different way of getting the bullet signatures in their paper [@cmps].}
