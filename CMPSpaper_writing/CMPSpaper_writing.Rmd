...
\wj{Note that in @cmps bullet signatures are sometimes referred as bullet profiles. However, to avoid confusion, we distinguish the notion of bullet signatures from bullet profiles. Bullet profiles are XXX, while bullet signatures are XXX.}
...
\wj{they didn't talk about a specific threshold or error rates}
...
\hh{XXX How about a barchart with results - or a teaser for an image in the back, that talks about the concept that a CMPS score should be high for two same-source signatures and low for different-source signatures}
\wj{further question?}

In the example in section "Implementation" we  see that two bullet signatures coming from the same source get a CMPS score of X, while two signatures coming from different sources get a CMPS score of Y. We also implemented graphing tools for users to better understand these results as well as the algorithm itself.
...
Conceptually, the CMPS algorithm consists of \hh{three main} steps:

1. **cut the reference profile into consecutive, non-overlapping and equal-length basis segments:**
<<<<<<< HEAD
the command `get_segs(x, len=50)` implements this first step. 
It takes a bullet signature \hh{XXX profile or signature?} \wj{further question?} in the format of a numeric vector and cuts it into consecutive, non-overlapping and equal-length segments of length `len`.
=======
>>>>>>> 9f10900a1f85c08fea5be31476117473b65fe1dc

2. **register each basis segment on the comparison profile:** 
Calculate the cross-correlation curve between a basis segment and the comparison profile.A registration position is considered as "good" if the basis segment achieves a high cross-correlation value at that position. 
The best registration position is decided upon using  a multi-peak inspection with or without different segment lengths:

    2.1. **multi-peak inspection (without different segment lengths):** each basis segment identifies positions of 5 (or multiple) highest peaks in the cross-correlation curve. Since false positive peaks would stand out when the segment length is short, positions of all 5 peak will be considered as the potential "best" registration position.
...
3. **determine the congruent registration position** with a small tolerance zone and the number of basis segments that take the congruent registration position as their "best" registration position. These segments are called the "congruent matching profile segments" (CMPS), and the number of CMPS is the CMPS score of this profile comparison.
...
* The command `get_segs(x, len=50)` implements step 1:  It takes a profile {XXX profile or signature?} in the format of a numeric vector and cuts it into consecutive, non-overlapping and equal-length segments of length `len`.
...
\hh{XXX Could we move the code into the corresponding figure and show the relevant pieces side by side? I'm thinking of examples similar to p.38 in https://journal.r-project.org/archive/2017/RJ-2017-023/RJ-2017-023.pdf}
```{r, eval=FALSE}
